...
class GitHubIssues:
    ...
    def get_or_create_today_issue(self) -> object:
        ...
        GitHub API のラベルフィルタは作成直後の Issue を返さないことがある（インデックス遅延）。
        ラベルフィルタで見つからない場合は、ラベルなしで全open issueを走査してタイトル一致を探す。
        """
        today = datetime.now(JST).strftime("%Y-%m-%d")
        title_prefix = f"【運用ループ】{today}"

        # 1次検索: ラベル＋state=openでフィルタ
        # ※ GitHub API は閉じたIssueを誤って返すケースがあるため、issue.state を必ず再確認する
        try:
            issues = self.repo.get_issues(state="open", labels=[self.DAILY_OP_LABEL])
            for issue in issues:
                if issue.title.startswith(title_prefix) and issue.state == "open":
                    logger.info(f"既存のIssueを使用: #{issue.number}")
                    return issue
        except Exception as e:
            logger.warning(f"ラベル付きIssue検索失敗: {e}")

        # 2次検索（防御的）: ラベルなしで open Issue を全走査
        # 直近作成された Issue が API インデックス遅延でラベル検索に出ないケースに対応
        try:
            all_open = self.repo.get_issues(state="open", sort="created", direction="desc")
            for issue in all_open:
                if issue.title.startswith(title_prefix) and issue.state == "open":
                    label_names = {l.name for l in issue.labels}
                    if self.DAILY_OP_LABEL in label_names:
                        logger.warning(f"ラベル検索では見つからなかったが、タイトル走査で発見: #{issue.number}（インデックス遅延の可能性）")
                        return issue
        except Exception as e:
            logger.warning(f"全Open Issue走査失敗: {e}")
...
""",
    ...
    def get_performance_logs(self, limit: int = 30) -> list:
        """
        「投稿実績ログ」ラベルのIssueを取得する（情報リサーチの分析用）
        """
