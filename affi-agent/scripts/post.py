...
def notify_post_error(slot_num: int, message: str):
    """投稿失敗時のDiscord通知（トークン期限切れ・プレースホルダー検知等）"""
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code >= 500:
                # 5xx は一時的障害として再試行
                logger.warning(f"Threadsコンテナ作成 5xx(試行{attempt+1}): {resp.status_code}")
                _time.sleep(5 * (attempt + 1))
                continue
            if resp.status_code >= 400:
                logger.error(f"Threadsコンテナ作成失敗: status={resp.status_code}, body={resp.text}, text_length={len(text)}")
            ...
            last_err = e
            logger.warning(f"Threadsコンテナ作成 一時失敗(試行{attempt+1}): {e}")
            _time.sleep(5 * (attempt + 1))
    logger.error(f"Threadsコンテナ作成 3回とも失敗: {last_err}")
...
def main():
    ...
    if args.publish_container:
        ...
        gh.add_comment(issue.number, f"""## 📤 投稿・計測より：投稿完了（手動公開）
...
""")
    ...
    if not post_text:
        msg = "SLOT_1の投稿テキストが取得できませんでした。"
        logger.error(msg)
        notify_post_error(1, msg + f"\nIssue: {issue.html_url}")
        sys.exit(1)

    # プレースホルダー検知（ライター/校閲で抽出失敗していた場合の最終防衛）
    placeholder_markers = ["抽出失敗", "（SLOT_", "ここに本題", "投稿内容なし", "TODO"]
    if any(m in post_text[:200] for m in placeholder_markers) or len(post_text.strip()) < 10:
        msg = f"SLOT_1 のテキストにプレースホルダー検出。投稿を中止。\n{post_text[:200]}"
        logger.error(msg)
        notify_post_error(1, msg + f"\nIssue: {issue.html_url}")
        sys.exit(1)

    # アフィリ投稿なら PR表記を確認・補完（親投稿は汚染しない）
    aff_signals = ["a.r10.to", "rakuten.co.jp", "amzn.to", "amazon.co.jp", "楽天で", "Amazon で", "プロモーション"]
    is_affiliate = any(s in post_text for s in aff_signals)
    has_pr = (
        any(tag in post_text for tag in ["#PR", "#広告", "[PR]", "【PR】", "#pr"])
        or any(line.strip() == "PR" for line in post_text.splitlines())
    )
    if is_affiliate and not has_pr:
        # ツリー投稿なら最終ブロックに PR を補完（親フック汚染を防ぐ）
        if "===THREAD===" in post_text:
            parts = post_text.split("===THREAD===")
            parts[-1] = f"\nPR\n{parts[-1].lstrip()}"
            post_text = "===THREAD===".join(parts)
        else:
            post_text = f"{post_text.rstrip()}\nPR"
        logger.info("アフィリ投稿と判定。最終ブロックに PR 行を補完しました。")
    ...
    if args.draft:
        ...
        comment_body = f"""## 📋 投稿・計測より：下書き保存完了
...
以下のコマンドを実行してください：
```bash
python scripts/post.py --publish-container {container_id}
```
...
"""
    ...
    comment_body = f"""## 📤 投稿・計測より：投稿完了{"（ツリー投稿）" if reply_ids else ""}
...
"""
    ...
    if DISCORD_WEBHOOK_URL:
        try:
            ...
            logger.warning(f"Discord通知失敗（スキップ）: {e}")

    logger.info("=== 投稿・計測 投稿実行完了 ===")
