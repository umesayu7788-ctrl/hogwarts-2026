...
def get_post_text_from_issue(issue_number: int, gh: GitHubIssues) -> str:
    ...
    for comment in reversed(comments):
        if "投稿・計測より：投稿完了" in comment.body:
...
def main():
    ...
    logger.info("=== 投稿・計測 エンゲージメント計測開始 ===")
    ...
    comment_body = f"""## 📊 投稿・計測より：エンゲージメント計測結果（24時間後）
...
### ▼ バズ判定
{'✅ **バズ投稿！** → buzz_posts.md に追記しました' if is_buzz else f'📝 通常投稿（いいね {likes} / 閾値 {BUZZ_THRESHOLD}、閲覧 {views} / 閾値 {BUZZ_VIEWS_THRESHOLD}）'}
