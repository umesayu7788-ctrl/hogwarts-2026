...
## Step 4.5: 会員認証の確認（Discord在籍 / U2）

認証はコミュニティ Discord サーバーへの**在籍確認**で行います。
月次のアクセスキーファイル（.key）は**廃止済み**で、ダウンロードや配置は不要です。

1. リポジトリ直下の `auth_member.txt` に、あなたのDiscordユーザーID（数字のみ）が入っているか確認
   （無ければ「始める」コマンドの最初にClaudeが聞いてくれるので、答えるだけでOK）
2. GitHub Actions で自動実行する場合は、リポジトリの Secret `DISCORD_USER_ID` にも同じIDを登録

> ⚠️ **コミュニティのDiscordに参加していないと、最初の認証チェックで止まります。**
