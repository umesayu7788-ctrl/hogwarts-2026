...
16. 親投稿: PR表記・URL禁止。商品名OK。体験ベース（80文字目安）
17. ツリー1: 続き+注意点+末尾に pr + 改行 + アフィリURL（100文字以上推奨）
18. 「治る」「効く」「必ず」「100%」「私のリンクから買って」禁止
19. 180文字ルールはSLOT_1・SLOT_2のみ。アフィリ日のSLOT_3ツリー1は100文字未満のときだけ差し戻し
"""
...
■ 審査で確認すべき語尾ルール:
- 許可される語尾: 「〜だよ」「〜んだ」「〜だね」「〜だった」「〜しよ」（声定義に書かれた語尾）
- 禁止語尾に「〜のさ」を含める（使われていたら差し戻し）
...
{affiliate_rules}
## 合格基準
- 声定義の語尾（「〜だよ」「〜んだ」「〜だね」「〜だった」「〜しよ」）が使われていればOK。「〜のさ」があれば差し戻し
- 感情フックが機能している
- 3スロットが別テーマ・別角度
