...
{opening_rule}
■ 絶対遵守ルール:
- 語尾は声定義に書かれたもの（「〜だよ」「〜んだ」「〜だね」「〜だった」「〜しよ」等）だけを使うこと。「〜のさ」は禁止
...
"""
    ...
    if post_type == "affiliate":
        system_instruction += """
■ アフィリ日の絶対ルール（SLOT_3）:
- SLOT_3だけ ===THREAD=== を1回だけ（親+ツリー1の2ブロック）。3個以上の ===THREAD=== をSLOT_3に書くのは禁止
- SLOT_1・SLOT_2は従来どおり3〜4ブロック（===THREAD=== 複数可）。PR・リンク禁止
- ツリー1の末尾は必ず「pr」単独行 + 改行 + アフィリURL
"""
...
- ❌ NG：「SNS投稿やデータ分析まで、\\n自動化できる。」
- ✅ OK：「発酵食品を1品増やしただけで、朝のだるさがマシになったんだよ。」
...
"""
    ...
    if post_type == "affiliate" and product:
        result = fix_affiliate_slot3_in_output(result, product)
...
def normalize_affiliate_slot3_body(body: str, url: str) -> str:
    """SLOT_3 を親+ツリー1の2ブロックに矯正し、pr+URLを付与"""
    parts = [p.strip() for p in body.split("===THREAD===") if p.strip()]
    if not parts:
        return body
    parent = parts[0]
    tree = "\n\n".join(parts[1:]) if len(parts) > 1 else ""
    if not tree:
        tree = "気になってる人は詳細見てみてね。"
...
def fix_affiliate_slot3_in_output(text: str, product: dict | None) -> str:
    """生成結果のSLOT_3セクションをアフィリ2ブロック構造に後処理"""
    if not product or "SLOT_3" not in text:
        return text
    url = (product.get("affiliate_url") or product.get("url", "")).strip()
    idx = text.find("SLOT_3")
    if idx == -1:
        return text
    section = text[idx:]
    lines = section.split("\n")
    ...
    if len(bars) < 2:
        return text
    body = "\n".join(lines[bars[0] + 1:bars[1]]).strip()
    fixed = normalize_affiliate_slot3_body(body, url)
    new_section = "\n".join(lines[:bars[0] + 1] + [fixed] + lines[bars[1]:])
